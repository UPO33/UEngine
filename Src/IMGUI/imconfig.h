...
#define IM_ASSERT(_EXPR)  UASSERT(_EXPR)

#ifdef IMGUI_SHARED
#ifdef IMGUI_BUILD
#define IMGUI_API UMODULE_EXPORT
#else
#define IMGUI_API  UMODULE_IMPORT
#endif
#else
#define IMGUI_API 
#endif
...
#define IM_VEC2_CLASS_EXTRA                                                 \
        ImVec2(const UCore::Vec2& f) { x = f.x; y = f.y; }                       \
        operator UCore::Vec2() const { return UCore::Vec2(x,y); }

#define IM_VEC4_CLASS_EXTRA                                                 \
        ImVec4(const UCore::Vec4& f) { x = f.x; y = f.y; z = f.z; w = f.w; }     \
        operator UCore::Vec4() const { return UCore::Vec4(x,y,z,w); }
