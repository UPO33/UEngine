...
#define ASSERT UASSERT

extern HWND UGetMainHWND();
...
#define WinRTKey(name) (unsigned char)Windows::System::VirtualKey::name
    ...
    void KbmInitialize()
    {
        ...
        if (FAILED(s_Keyboard->SetCooperativeLevel(UGetMainHWND(), DISCL_FOREGROUND | DISCL_NONEXCLUSIVE)))
        ...
        if (FAILED(s_Mouse->SetCooperativeLevel(UGetMainHWND(), DISCL_BACKGROUND | DISCL_NONEXCLUSIVE)))
    ...
    void KbmUpdate()
    {
        ...
        if (foreground != UGetMainHWND() // wouldn't be able to acquire
            || !visible)
        {
            KbmZeroInputs();
        }
