...
void UParseConfig1(const char* str, ConfNode& nodeOut)
{
	//read and trim white spaces
	auto ReadTrim = [](const char*& str)
	{
		while (char chr = *str)
		{
			if (lexertk::details::is_whitespace(chr))
				str++;
			else
				break;
		}
	...
	auto ReadName = [](const char* str, char* outName) -> bool
	{
		*outName = '\0';

		while (char chr = *str)
		{
			if (lexertk::details::is_letter_or_digit(chr))
			{
				*outName = chr;
				str++;
				outName++;
...
void UParseConfig(std::string expression, ConfNode& nodeOut)
{
	nodeOut = ConfNode();
	...
	for (int i = 0; i < generator.size(); i++)
	{
		...
		if (t.type == lexertk::token::e_symbol)
		{
		...
		{
			ConfNode childNode;
			childNode.mIsValid = true;
			childNode.mName = t.value;
		}
	}
...
//    std::string expression = "(sin(x/pi)cos(2y) + 1) == (sin(x / pi) * cos(2 * y) + 1)";
	std::string expression = "{a=true}";
	std::string expression1 = R"({ 
	Re solution = {800, 600},
	B = true,
	Size = {24, 24, 66},
	Name ='sd  f'
